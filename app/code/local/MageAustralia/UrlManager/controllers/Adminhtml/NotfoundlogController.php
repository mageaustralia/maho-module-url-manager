 ...
 * @package    MageAustralia_UrlManager
 */
class MageAustralia_UrlManager_Adminhtml_NotfoundlogController extends Mage_Adminhtml_Controller_Action
{
    public const ADMIN_RESOURCE = 'catalog/mageaustralia_urlmanager/notfound_log';
    ...
    {
        $this->loadLayout()
            ->_setActiveMenu('mageaustralia_urlmanager/notfoundlog')
            ->_addBreadcrumb(
                Mage::helper('mageaustralia_urlmanager')->__('Notfound_log'),
                Mage::helper('mageaustralia_urlmanager')->__('Notfound_log'),
            );
        return $this;
    }
    ...
    public function editAction(): void
    {
        ...
        $model = Mage::getModel('mageaustralia_urlmanager/notfoundlog');

        if ($id) {
            $model->load($id);
            if (!$model->getId()) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('mageaustralia_urlmanager')->__('Notfound_log does not exist'),
                );
                $this->_redirect('*/*/');
                return;
            }
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('mageaustralia_urlmanager')->__('Notfound_log was successfully saved'),
                );
    ...
    public function deleteAction(): void
    {
        if ($id = $this->getRequest()->getParam('id')) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('mageaustralia_urlmanager')->__('Notfound_log was successfully deleted'),
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    public function massDeleteAction(): void
    {
        ...
        if (!is_array($ids)) {
            ...
            try {
                foreach ($ids as $id) {
                    $model = Mage::getModel('mageaustralia_urlmanager/notfoundlog')->load($id);
                    $model->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(

 ...
 * @package    MageAustralia_UrlManager
 */
class MageAustralia_UrlManager_Model_Csv_Import
{
    /**
    ...
    {
        Mage::log('CSV Import Model: Starting import from: ' . $filePath, Mage::LOG_INFO, 'mageaustralia_urlmanager.log');
        Mage::log('CSV Import Model: Skip duplicates: ' . ($skipDuplicates ? 'yes' : 'no'), Mage::LOG_DEBUG, 'mageaustralia_urlmanager.log');

        if (!file_exists($filePath)) {
            Mage::log('CSV Import Model: ERROR - File does not exist!', Mage::LOG_ERR, 'mageaustralia_urlmanager.log');
        ...
        Mage::log('CSV Import Model: Delimiter: ' . $delimiter . ', Enclosure: ' . $enclosure, Mage::LOG_DEBUG, 'mageaustralia_urlmanager.log');
        ...
        try {
            while (($data = fgetcsv($handle, 0, $delimiter, $enclosure)) !== false) {
                ...
                Mage::log('CSV Import Model: Line ' . $lineNumber . ' raw data: ' . print_r($data, true), Mage::LOG_DEBUG, 'mageaustralia_urlmanager.log');

                // Skip header row if it exists
                if ($lineNumber === 1 && strtolower((string) $data[0]) === 'old_url') {
                    Mage::log('CSV Import Model: Skipping header row', Mage::LOG_DEBUG, 'mageaustralia_urlmanager.log');
                    continue;
                }
                ...
                try {
                    ...
                    Mage::log('CSV Import Model: Line ' . $lineNumber . ' result: ' . $rowResult, Mage::LOG_DEBUG, 'mageaustralia_urlmanager.log');
    ...
    protected function importRow(array $data, bool $skipDuplicates): string
    {
        ...
        $sourceUrl = trim((string) $data[0]);
        $destinationUrl = trim((string) $data[1]);
        ...
        /** @var MageAustralia_UrlManager_Model_Resource_Redirect_Collection $collection */
        ...
        // Create new redirect
        $redirect = Mage::getModel('mageaustralia_urlmanager/redirect');
        $redirect->setData([

 ...
 * @package    MageAustralia_UrlManager
 */
class MageAustralia_UrlManager_Model_Observer
{
    /**
    ...
    public function initRouters(\Maho\Event\Observer $observer): void
    {
        ...
        $router = new MageAustralia_UrlManager_Controller_Router();
        $observer->getEvent()->getFront()->addRouter('mageaustralia_urlmanager', $router);

        Mage::log('URL Manager Observer: Router added successfully', Mage::LOG_INFO, 'mageaustralia_urlmanager.log');
    }
    ...
    public function logNotFound(\Maho\Event\Observer $observer): void
    {
        ...
        if ($collection->getSize() > 0) {
            ...
            /** @var MageAustralia_UrlManager_Model_Notfoundlog $log */
        ...
        if ($collection->getSize() > $maxEntries) {
            ...
            if (!empty($idsToDelete)) {
                ...
                Mage::getSingleton('core/resource')->getConnection('core_write')
                    ->delete(
                        Mage::getResourceModel('mageaustralia_urlmanager/notfoundlog')->getMainTable(),
    ...
    protected function sendReport(string $period): void
    {
        ...
        if (empty($recipientEmail)) {
            Mage::log('Cannot send 404 report: No recipient email configured', Mage::LOG_WARNING, 'mageaustralia_urlmanager.log');
            return;
        }
        ...
        if (empty($top404s)) {
            Mage::log(sprintf('No new 404s since last %s report — skipping email', $period), Mage::LOG_INFO, 'mageaustralia_urlmanager.log');
            return;
        }
        ...
        try {
            ...
            // Load template file
            $templateFile = Mage::getBaseDir('locale') . DS . 'en_US' . DS . 'template' . DS . 'email' . DS . 'mageaustralia_urlmanager' . DS . '404_report.html';

            if (!file_exists($templateFile)) {
                Mage::log('404 report email template not found: ' . $templateFile, Mage::LOG_ERROR, 'mageaustralia_urlmanager.log');
                return;
            }
            ...
            if (!$sent) {
                Mage::log('Failed to send 404 report email', Mage::LOG_ERROR, 'mageaustralia_urlmanager.log');
            } else {
                ...
                if (!empty($reportedIds)) {
                    ...
                    $writeAdapter->update(
                        Mage::getSingleton('core/resource')->getTableName('mageaustralia_urlmanager/notfoundlog'),
